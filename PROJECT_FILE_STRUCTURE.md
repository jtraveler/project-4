...
**Current Phase:** Phase REP (Replicate + xAI providers — Session 154; gpt-image-2 BYOK integration added Session 171-C; Grok content_policy keyword expansion + per-image overlay restoration in Session 172; per-card reset bug fixes + NSFW pre-flight v1 + content_policy chip icon in Session 173; Tier 2 advisory pre-flight activated end-to-end in Session 173-E; chip redesign + Tier 2 architectural fix + block_source distinction + report-to-admin mailto stub in Session 173-F). Bulk AI Image Generator (Phases 1–7 + 6E complete). Avatar pipeline rebuilt B2-native (Session 163). Phase N4 (~100%), Phase K (~96%)
**Total Tests:** 1414 passing, 12 skipped (Session 173-F)
...
| **Migrations** | 94 | prompts/migrations/ (92, latest `0092_profanityword_provider_aware` — Session 173-B, AddField for `block_scope`/`affected_providers`/`last_reviewed_at`/`review_notes` on ProfanityWord enabling tiered NSFW pre-flight; `0091_add_gpt_image_2_to_choices` — Session 171-C, auto-generated AlterField on Prompt+DeletedPrompt for AI_GENERATOR_CHOICES tuple; `0090_seed_gpt_image_2` — Session 171-C RunPython data migration seeding gpt-image-2-byok GeneratorModel row; `0089_add_error_type_retry_count_to_generatedimage` — Session 170-A retry-state schema additions; 169-C added `0088_alter_deletedprompt_ai_generator` schema-only RegexValidator addition; 169-B added `0087_retag_grok_prompts` data migration retagging 7 mis-tagged Grok prompts to `grok-imagine`), about/migrations/ (2) |
