...
**Version:** 4.72 (Session 173-E SINGLE-SPEC follow-on cluster — frontend wire-up of `model_identifier` in `api_validate_prompts` POST body. Activates 173-B Tier 2 advisory pre-flight in production. Closes REPORT_173_B Section 5 P2 deferred item per Memory Rule #16. **Step 0 finding:** backend wire-up was ALREADY DONE in 173-B (`bulk_generator_views.py:267-275`); only frontend gap remained. One-line fix at `bulk-generator-generation.js:911` adds `model_identifier: (I.settingModel && I.settingModel.value) || ''` to the validate fetch body. Defensive empty-string fallback preserves backward-compat (Tier 1 universal-only when empty, the 173-B-defined contract — not silent corruption, so Memory Rule #13 N/A). **3 new defensive tests** in `ValidatePromptsAPITests` (happy path, backward-compat, malformed-coerce-to-empty). All 11 ValidatePromptsAPITests pass. **Cluster shape:** SINGLE-SPEC. **Memory Rule #17 single-commit pattern:** docs and code ship in same commit; "see commit log for hash" phrasing avoids self-referencing chicken-and-egg. 1411 tests.)
