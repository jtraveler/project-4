...
(function () {
    ...
    I.generateBtn.addEventListener('click', function () {
        ...
        var finalPrompts = prompts.map(function (p, i) {
            // Vision-enabled boxes have empty textarea text by design —
            // their prompt is generated by the Vision API in the prepare step.
            // Use a placeholder so server-side validation accepts them.
            var isVision = visionEnabled && visionEnabled[i];
            // Vision boxes ALWAYS use placeholder — any existing textarea
            // text is intentionally ignored. The Vision API generates the
            // real prompt during prepare-prompts.
            var text = isVision ? '[Vision prompt ' + (i + 1) + ' — to be generated]' : p;
            return charDesc ? charDesc + '. ' + text : text;
        });
